...
func mkfs(id string) string {
	p := ""
	particion := GetMount("MKFS", id, &p)
	n := math.Floor(float64(particion.Part_s-int64(unsafe.Sizeof(Structs.SuperBloque{}))) / float64(4+unsafe.Sizeof(Structs.Inodos{})+3*unsafe.Sizeof(Structs.BloquesArchivos{})))
