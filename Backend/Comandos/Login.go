...
func ValidarDatosLOGIN(context []string) bool {
	...
	for i := 0; i < len(context); i++ {
		...
		if Comparar_Cadenas(tk[0], "id") {
			id = tk[1]
		} else if Comparar_Cadenas(tk[0], "usuario") {
			user = tk[1]
		} else if Comparar_Cadenas(tk[0], "password") {
			pass = tk[1]
		}
	}
	if id == "" || user == "" || pass == "" {
	...
	return sesion_Activa(user, pass, id)
}

func sesion_Activa(u string, p string, id string) bool {
	var path string
	...
	for i := 0; i < len(vctr)-1; i++ {
		...
		if linea[2] == 'U' || linea[2] == 'u' {
			...
			if Comparar_Cadenas(in[3], u) && Comparar_Cadenas(in[4], p) && in[0] != "0" {
				idGrupo := "0"
				existe := false
...
func CerrarSesion() bool {
	Mensaje("LOGOUT", "Usuario: "+Logged.User+" - Sesion Terminada")
